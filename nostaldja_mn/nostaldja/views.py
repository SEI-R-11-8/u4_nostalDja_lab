...
def show_base(request):
    base = Decade.objects.all()
    return render(request, 'nostaldja/base.html', {'base': base})


def decade_all(request):
    decades = Decade.objects.all()
    return render(request, 'nostaldja/decade_all.html', {'decades': decades})


def fad_all(request):
    fads = Fad.objects.all()
    return render(request, 'nostaldja/fad_all.html', {'fads': fads})
...
def decade_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            decade = form.save()
            return redirect('decade_all', pk=decade.pk)
    else:
...
def fad_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            fad = form.save()
            return redirect('fad_all', pk=fad.pk)
    else:
